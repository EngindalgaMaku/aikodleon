...
export async function generateMetadata(): Promise<Metadata> {
  const title = "Yasaklı Arama (Tabu Search)";
  const description = "Yasaklı Arama (Tabu Search) metasezgisel optimizasyon algoritmasının temellerini, yasaklı listesi mekanizmasını ve uygulama alanlarını keşfedin.";
  return {
    title: `${title} | Metasezgisel Optimizasyon | Kodleon`,
    description: description,
    keywords: "yasaklı arama, tabu search, ts, metasezgisel optimizasyon, yerel arama, hafıza mekanizmaları, optimizasyon",
    openGraph: {
      title: `${title} | Kodleon`,
      description: description,
      images: [{ url: 'https://images.pexels.com/photos/163064/play-ground-rainbow-colors-163064.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2' }],
    }
  };
}

export default async function TabuSearchPage() {
  const markdownFilePath = 'topics/metasezgisel-optimizasyon/yasakli-arama.md';
  const markdownContent = await getPageContent(markdownFilePath);
  const pageTitle = "Yasaklı Arama";
      ...
      <div className="container mx-auto py-10 px-4 sm:px-6 lg:px-8 max-w-5xl">
        <div className="mb-8 flex justify-between items-center">
          <Button asChild variant="outline" size="sm" className="gap-1 text-sm">
            <Link href="/topics/metasezgisel-optimizasyon/karinca-kolonisi-optimizasyonu">
              <ArrowLeft className="h-4 w-4" />
              Önceki: Karınca Kolonisi Optimizasyonu
            </Link>
          </Button>
          <Button asChild variant="outline" size="sm" className="gap-1 text-sm">
            <Link href="/topics/metasezgisel-optimizasyon">
        ...
        <div className="mt-12 text-center">
          <Button asChild variant="default">
            <Link href="/topics/metasezgisel-optimizasyon/yapay-ari-kolonisi-algoritmasi">
              Sonraki Konu: Yapay Arı Kolonisi Algoritması <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </div>
