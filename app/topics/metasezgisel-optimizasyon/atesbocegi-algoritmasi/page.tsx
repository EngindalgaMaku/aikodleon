...
export async function generateMetadata(): Promise<Metadata> {
  const title = "Ateşböceği Algoritması (Firefly Algorithm - FA)";
  const description = "Ateşböceği Algoritması (FA), ateşböceklerinin parıldama davranışlarından esinlenen, çekicilik ve ışık yoğunluğu prensiplerine dayalı metasezgisel optimizasyon algoritmasıdır.";
  return {
    title: `${title} | Metasezgisel Optimizasyon | Kodleon`,
    description: description,
    keywords: "ateşböceği algoritması, firefly algorithm, fa, metasezgisel optimizasyon, parıldama davranışı, çekicilik, ışık yoğunluğu, optimizasyon",
    openGraph: {
      title: `${title} | Kodleon`,
      description: description,
      images: [{ url: 'https://images.pexels.com/photos/414012/pexels-photo-414012.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2' }],
    }
  };
}

export default async function FireflyAlgorithmPage() {
  const markdownFilePath = 'topics/metasezgisel-optimizasyon/atesbocegi-algoritmasi.md';
  const markdownContent = await getPageContent(markdownFilePath);
  const pageTitle = "Ateşböceği Algoritması";
        ...
        <div className="mt-12 text-center">
          <Button asChild variant="default">
            <Link href="/topics/metasezgisel-optimizasyon/guguk-kusu-aramasi">
              Sonraki Konu: Guguk Kuşu Araması <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </div>
