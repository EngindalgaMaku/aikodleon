...
export async function generateMetadata(): Promise<Metadata> {
  const title = "Guguk Kuşu Araması (Cuckoo Search - CS)";
  const description = "Guguk kuşlarının yumurtlama stratejisini ve yumurta tanıma mekanizmasını taklit eden, Lévy uçuşlarını kullanan metasezgisel optimizasyon algoritması.";
  return {
    title: `${title} | Metasezgisel Optimizasyon | Kodleon`,
    description: description,
    keywords: "guguk kuşu araması, cuckoo search, cs, metasezgisel optimizasyon, lévy uçuşları, yumurtlama stratejisi, optimizasyon",
    openGraph: {
      title: `${title} | Kodleon`,
      description: description,
      images: [{ url: 'https://images.pexels.com/photos/2313396/pexels-photo-2313396.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2' }],
    }
  };
}

export default async function CuckooSearchPage() {
  const markdownFilePath = 'topics/metasezgisel-optimizasyon/guguk-kusu-aramasi.md';
  const markdownContent = await getPageContent(markdownFilePath);
  const pageTitle = "Guguk Kuşu Araması";
        ...
        <div className="mt-12 text-center">
          <Button asChild variant="default">
            <Link href="/topics/metasezgisel-optimizasyon/gri-kurt-optimizasyonu">
              Sonraki Konu: Gri Kurt Optimizasyonu <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </div>
