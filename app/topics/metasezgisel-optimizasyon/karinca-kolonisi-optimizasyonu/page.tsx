...
export async function generateMetadata(): Promise<Metadata> {
  const title = "Karınca Kolonisi Optimizasyonu (Ant Colony Optimization - ACO)";
  const description = "Karınca Kolonisi Optimizasyonu (ACO) algoritmasının çalışma prensiplerini, feromon izlerini, avantajlarını ve optimizasyon problemlerindeki uygulamalarını keşfedin.";
  return {
    title: `${title} | Metasezgisel Optimizasyon | Kodleon`,
    description: description,
    keywords: "karınca kolonisi optimizasyonu, aco, metasezgisel optimizasyon, feromon, yapay zeka, kolektif zeka, optimizasyon",
    openGraph: {
      title: `${title} | Kodleon`,
      description: description,
      images: [{ url: 'https://images.pexels.com/photos/790357/pexels-photo-790357.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2' }],
    }
  };
}

export default async function AntColonyOptimizationPage() {
  const markdownFilePath = 'topics/metasezgisel-optimizasyon/karinca-kolonisi-optimizasyonu.md';
  const markdownContent = await getPageContent(markdownFilePath);
  const pageTitle = "Karınca Kolonisi Optimizasyonu";
      ...
      <div className="container mx-auto py-10 px-4 sm:px-6 lg:px-8 max-w-5xl">
        ...
        <div className="mt-12 text-center">
          <Button asChild variant="default">
            <Link href="/topics/metasezgisel-optimizasyon/yasakli-arama">
              Sonraki Konu: Yasaklı Arama <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </div>
